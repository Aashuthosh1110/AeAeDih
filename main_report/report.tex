...
\section{Abstract}
This report presents a from-scratch implementation of the Miller-Rabin probabilistic primality testing algorithm in C++. The Miller-Rabin algorithm achieves $O(k \log^3 n)$ time complexity, vastly superior to Trial Division's $O(\sqrt{n})$ for large integers.

\section{Algorithm Description}

\subsection{Theoretical Foundation}
The Miller-Rabin test is a randomized Monte Carlo algorithm. It improves upon Fermat's test by checking for non-trivial square roots of unity modulo $n$.
For odd integer $n$, express $n-1 = 2^r \cdot d$. For random base $a$:
\begin{enumerate}
...
\textbf{Error Probability:} $P(\text{Error in } k \text{ rounds}) \le 4^{-k}$.

\subsection{Complexity}
\begin{itemize}
    \item \textbf{Time Complexity:} $T(n) = O(k \log^3 n)$. This polylogarithmic complexity enables instant primality testing of large numbers.
    \item \textbf{Space Complexity:} $O(1)$ auxiliary space.
\end{itemize}

\section{Implementation Details}
\begin{itemize}
    \item \textbf{Overflow Protection:} Uses \texttt{unsigned \_\_int128} casting for intermediate modular multiplications to prevent 64-bit overflow.
    \item \textbf{RNG:} Used \texttt{std::mt19937\_64} for high-quality random witness selection.
    \item \textbf{Modular Exponentiation:} Square-and-Multiply algorithm used for $O(\log \text{exp})$.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Trial Division:} Exhibits exponential growth ($O(\sqrt{n})$). Hit a "wall" at 48+ bits.
    \item \textbf{Miller-Rabin:} Maintains flat $O(k \log^3 n)$ profile. Microsecond-level execution for 64-bit inputs.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Runtime comparison: Trial Division vs Miller-Rabin}
    \label{fig:mr_runtime}
\end{figure}

\subsection{Error Rate Analysis: Carmichael Numbers}
Tested on 5 Carmichael numbers (561, 1105, 1729, 2465, 6601).
\textbf{Key Finding:} All converged to 0\% error by $k=4$, far exceeding the theoretical guarantee of $4^{-k}$.
...
% ===================================================================
\chapter{Analysis of Randomized Incremental Construction for Convex Hulls}
...
In domains like cryptography, deterministic solutions are often theoretically possible but practically useless. As shown in our Miller-Rabin analysis, deterministic Trial Division hit a wall at 48 bits, while the randomized approach scaled effortlessly to 64 bits and beyond. Here, the trade-off of a $4^{-k}$ error rate is statistically negligible compared to the benefit of actually being able to compute the result.
